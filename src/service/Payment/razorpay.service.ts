...
const RAZORPAY_KEY_ID = config.get<string>("RAZORPAY_KEY_ID");
const RAZOR_PAY_KEY_SECRET = config.get<string>("RAZOR_PAY_KEY_SECRET");
...
export const createOrderRazerPay = async (
  ...
  const res = await instance.orders.create({
    amount: parseInt(String(amount * 100)),
    currency,
    receipt,
  });
  return res;
...
export const createPlan = async ({
  ...
  const newPlan = await instance.plans.create({
    ...
    item: {
      ...
      amount: parseInt(toString(amount * 100)),
      currency: "INR",
    },
    notes: {
...
export const createNewSubcription=async({planId,phoneNo}:{planId:string,phoneNo:string})=>{
   const subcription=await instance.subscriptions.create({
     plan_id:planId,
     customer_notify:1,total_count:12,
